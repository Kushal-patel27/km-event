...
export default function FeatureToggles() {
  ...
  const fetchEventDetails = async () => {
    // Event title is now fetched in fetchFeatures
  }
  ...
  if (loading) {
    return (
      <SuperAdminLayout title="Feature Management" subtitle="Loading event details...">
        <div className="min-h-[40vh] flex items-center justify-center text-gray-600">Loading features...</div>
      </SuperAdminLayout>
    )
  }
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto pb-10">
            ...
            {featuresList.map((feature) => (
              ...
              >
                <div className="flex-1">
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{feature.label}</h3>
        ...
        {/* Summary */}
        <div className={`mt-8 rounded-lg p-6 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} border-2 ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <h3 className={`font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Enabled Features Summary</h3>
