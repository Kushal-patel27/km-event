...
export default function Booking(){
  ...
  return (
    <div className={`min-h-screen py-12 transition-colors ${isDarkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-3xl mx-auto px-6">
        <div className={`rounded-2xl shadow-lg overflow-hidden transition-colors ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          {/* Event Header */}
          <div className={`text-white p-6 transition-colors ${
            isDarkMode 
          ...
          {/* Form */}
          <form onSubmit={handleSubmit} className={`p-6 md:p-8 space-y-6 transition-colors ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
            {/* Personal Info */}
            <div className="space-y-4">
              <h3 className={`text-lg font-semibold flex items-center gap-2 transition-colors ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className={`block text-sm font-medium mb-1.5 transition-colors ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Full Name</label>
                  <input 
                    required 
                    value={name} 
                    onChange={e=>setName(e.target.value)} 
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-400' : 'border-gray-300 bg-white text-gray-900 placeholder-gray-600'}`}
                    placeholder="John Doe"
                  />
                </div>
                <div>
                  <label className={`block text-sm font-medium mb-1.5 transition-colors ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Email Address</label>
                  <input 
                    required 
                    type="email" 
                    value={email} 
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <label className={`block text-sm font-medium mb-1.5 transition-colors ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Number of Tickets</label>
                  <input 
                    required 
                    type="number" 
                    min="1" 
                    ...
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'border-gray-300 bg-white text-gray-900'}`}
                  />
                </div>
                <div className={`flex-1 text-right pt-6 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${isDarkMode ? 'bg-red-900 text-red-200' : 'bg-indigo-50 text-indigo-700'}`}>
            ...
            {hasSeatLayout && (
              <div>
                ...
                <div className={`p-8 rounded-lg border-4 transition-colors ${isDarkMode ? 'bg-gradient-to-b from-red-950 to-red-900 border-red-900' : 'bg-gradient-to-b from-blue-900 to-blue-800 border-blue-950'}`}>
                  <div className="mb-6">
            ...
            {/* Total & Actions */}
            <div className={`border-t pt-6 space-y-4 transition-colors ${isDarkMode ? 'border-gray-700' : ''}`}>
              ...
              <div className={`flex items-baseline justify-between text-2xl pt-3 border-t transition-colors ${isDarkMode ? 'border-gray-700' : ''}`}>
                <span className={`font-bold transition-colors ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Total</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <Link 
                to={`/event/${event.id}`} 
                className={`flex-1 px-6 py-3 border-2 rounded-lg font-semibold transition text-center ${isDarkMode ? 'border-red-600 text-red-300 hover:bg-red-700/20' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
              >
                ← Back to Event
              </Link>
              <button 
