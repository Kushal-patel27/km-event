...
export default function BookingSearchDashboard() {
  ...
  const loadBookingDetails = async (item) => {
    try {
      ...
      if (item?.bookingId) {
        const fullRes = await API.get(`/bookings/admin/search-booking?bookingId=${encodeURIComponent(item.bookingId)}`)
      ...
      if (fallbackTicketId) {
        const fullRes = await API.get(`/bookings/admin/search-ticket?ticketId=${encodeURIComponent(fallbackTicketId)}`)
        setBooking(fullRes.data.booking)
        return
      }
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (searchType === 'bookingId') {
        ...
        res = await API.get(`/bookings/admin/search-user?search=${encodeURIComponent(searchTerm)}&page=1&limit=50`)
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {booking && (
          <div className="space-y-6">
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-lg shadow-lg p-4 sm:p-6 lg:p-8 text-white">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
