...
export default function EventCard({event}){
  ...
  return (
    <div className={`backdrop-blur-xl rounded-2xl shadow-xl overflow-hidden transform transition hover:-translate-y-2 border ${
      isDarkMode
      ...
      <div className="relative w-full aspect-video overflow-hidden group">
        ...
        <div className={`absolute top-3 left-3 backdrop-blur-sm text-white text-xs font-semibold px-3 py-1.5 rounded-full ${
          isDarkMode ? 'bg-red-600/95 shadow-lg shadow-red-600/30' : 'bg-indigo-600/95 shadow-lg shadow-indigo-600/30'
      ...
      <div className="p-5">
        ...
        <div className={`flex items-center justify-between gap-3 pt-4 border-t ${isDarkMode ? 'border-white/15' : 'border-gray-200'}`}>
          <div>
            <div className={`font-bold text-xl ${isDarkMode ? 'text-red-500' : 'text-indigo-600'}`}>{formatINR(event.price)}</div>
            <Link to={`/event/${eventId}`} className={`text-xs ${isDarkMode ? 'text-gray-400 hover:text-red-400' : 'text-gray-500 hover:text-indigo-600'} hover:underline transition`}>View details →</Link>
          </div>
