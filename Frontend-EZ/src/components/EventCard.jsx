      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-gray-500">{available > 0 ? `${available} left` : 'Sold out'}</div>
          </div>
        </div>

        <div className="mt-4 flex items-center justify-between">
          <Link to={`/event/${event.id}`} className="text-sm text-gray-600 hover:underline">Details</Link>
          <Link to={`/book/${event.id}`} className={`inline-block text-sm px-4 py-2 rounded-md ${available>0? 'bg-gradient-to-r from-indigo-600 to-blue-500 text-white' : 'bg-gray-200 text-gray-500 pointer-events-none'}`}>Book</Link>
        </div>
      </div>
    </div>
  )
