...
export default function NavigationButtons({
  ...
  const sizeClasses = size === 'sm'
    ? 'h-9 w-9 p-0 text-xs leading-none'
    : 'px-3 py-2 text-sm'

  const baseButton = `inline-flex items-center justify-center gap-1 rounded-lg border border-gray-200 bg-white text-gray-700 hover:bg-gray-100 transition ${sizeClasses}`
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <button
        ...
        aria-label="Go back"
      >
        <svg className="block h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
