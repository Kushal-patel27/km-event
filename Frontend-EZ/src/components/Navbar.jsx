          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {user && user.isAdmin && (
              <Link
                ...
                className={`font-medium transition ${location.pathname.startsWith('/admin') || location.pathname.startsWith('/event-admin') || location.pathname.startsWith('/staff') ? isHomePage ? 'text-red-500' : 'text-red-600 dark:text-red-500' : isHomePage ? 'text-gray-200 hover:text-red-500' : 'text-gray-700 dark:text-gray-300 hover:text-red-600 dark:hover:text-red-500'}`}
              >
                Admin
              </Link>
            )}
            ...
            <nav className="flex flex-col space-y-2">
              ...
              {user && user.isAdmin && (
                <Link
                  to="/admin"
                  onClick={() => setMobileMenuOpen(false)}
                  className={`px-4 py-2 rounded-lg transition ${location.pathname.startsWith('/admin') ? isHomePage ? 'bg-red-900/30 text-red-500 font-semibold' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-500 font-semibold' : isHomePage ? 'text-gray-200 hover:bg-gray-700' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                >
                  Admin
                </Link>
              )}
