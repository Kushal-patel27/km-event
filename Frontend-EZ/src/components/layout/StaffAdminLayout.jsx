...
export default function StaffAdminLayout({ title = 'Staff Admin', children }){
  ...
  const nav = [
    { to: '/staff-admin/dashboard', label: 'Dashboard', icon: '📊' },
    { to: '/staff-admin/team', label: 'Team Members', icon: '👥' },
    { to: '/staff-admin/entries', label: 'Entry Logs', icon: '📋' },
    { to: '/staff-admin/settings', label: 'Settings', icon: '⚙️' },
  ]
  ...
  return (
    <div className="responsive-role-shell role-shell-staff-admin min-h-screen bg-gray-50 text-gray-900">
      {/* Top Bar */}
          ...
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 md:gap-4 min-w-0">
              ...
              <Link to="/" className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
                <span className="md:hidden"><Logo dark={false} size="lg" key="staff-admin-logo-sm" /></span>
                <span className="hidden md:block"><Logo dark={false} size="sm" key="staff-admin-logo" /></span>
              </Link>
              <div className="hidden md:block border-l border-gray-200 h-6"></div>
              <h1 className="text-xs sm:text-sm md:text-base lg:text-lg font-semibold truncate">Staff Admin Panel</h1>
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2 md:gap-3 flex-shrink-0">
              ...
              <div className="hidden sm:flex items-center gap-3 md:gap-4">
                <div className="flex flex-col items-end text-right">
                  <p className="text-xs sm:text-sm font-medium">{user?.name || 'Staff Admin'}</p>
                  <p className="text-[10px] sm:text-xs text-gray-500">Staff Administrator</p>
                </div>
              </div>
              <button 
